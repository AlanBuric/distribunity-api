<script lang="ts" setup>
  ...
  const UNSAVED_CHANGES_MESSAGE =
    'You have unsaved changes. Please save or discard the current item being edited before selecting a new inventory or item.';
  ...
  const selectedInventoryIndex = ref<number | undefined>(
    inventories.value.length > 0 ? 0 : undefined,
  );
  ...
  const selectedInventory = computed(() =>
    selectedInventoryIndex.value != null
      ? inventories.value[selectedInventoryIndex.value]
      : undefined,
  );
  ...
  function createNewInventory() {
    ...
    if (inventoryName) {
      addDoc(collection(database, 'organizations', organizationId, 'inventories'), {
        name: inventoryName,
        items: [],
      });
    }
  ...
  async function renameInventory(index: number) {
    ...
    if (inventory) {
      ...
      if (newName) {
        updateDoc(doc(database, 'organizations', organizationId, 'inventories', inventory.id), {
          name: newName,
        });
      }
    }
  ...
  function deleteItem(item: Item & WithId) {
    ...
    if (isSelectedItem && activeItemPanel.value == 'editor') {
      ...
      confirm(`Are you sure you want to delete ${item.name} from ${selectedInventory.value.name}?`)
    ) {
      if (isSelectedItem) {
        selectedItem.value = undefined;
      }
...
<template>
  <div class="h-full flex flex-row items-stretch">
    ...
    <div class="flex flex-col flex-1 items-stretch">
      <WorkToolbar
        @new-item="tryOpenItemCreator"
        :has-inventories="hasInventories()"
        class="flex-initial"
      />
      <div class="flex flex-1 gap-3 mx-2 mt-2 content-stretch">
        <main class="flex-1 bg-white dark:bg-slate-800 rounded-t-lg">
          <h2 class="p-3 text-semibold text-slate-500 dark:text-slate-400">
            ...
            >
            <RouterLink
              :to="`/work/organization/${organizationId}`"
              class="underline"
              title="Visit this organization's admin page"
            >
              {{ authStore.currentOrganization?.name ?? 'Unknown' }}
            </RouterLink>
            >
            <RouterLink
              :to="`/work/organization/${organizationId}/inventories`"
              class="underline"
              title="You're already here"
            >
              Inventories
            </RouterLink>
            <span v-if="selectedInventory">> {{ selectedInventory.name }}</span>
          </h2>
          <ItemTable
