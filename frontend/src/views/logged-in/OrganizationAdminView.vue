<script setup lang="ts">
  ...
  async function saveOrganizationSettings() {
    if (!authStore.hasPermission('organization.edit')) {
      ...
      await updateDoc(organizationDocRef, {
        ...
        name: newName.value ?? organization.value?.name ?? 'Unknown organization',
      });
  ...
  function removeMember(user: User & WithId) {
    if (!authStore.hasPermission('organization.invites.create')) {
      ...
      confirm(
        `Are you sure you want to kick ${user.firstName} ${user.lastName} from your organization?`,
      )
    ) {
      const batch = writeBatch(database);

      batch.delete(doc(membersCollRef, user.id));
      batch.update(doc(database, 'users', user.id), {
        organizations: arrayRemove(organizationDocRef),
      });
    ...
    <div class="max-w-5xl flex-grow flex flex-col items-center py-6 space-y-6">
      ...
      <section class="w-full bg-white dark:bg-slate-800 shadow rounded-lg px-5 py-4">
        ...
        <form @submit.prevent="saveOrganizationSettings" class="flex flex-col space-y-4">
          ...
          <button
            type="submit"
            class="flex-initial px-3 py-2 bg-teal-500 text-white rounded"
      ...
      <section class="w-full bg-white dark:bg-slate-800 shadow rounded-lg px-5 py-4 space-y-3">
        ...
        <ul v-if="organization.invites.length < 20">
          <li
            v-for="(invite, index) in organization.invites"
            :key="index"
            class="flex justify-between items-center mb-2"
          >
            <span>{{ invite }}</span>
            <button
              @click="removeInviteCode(invite)"
        ...
        <p v-else>
          <span v-for="(invite, index) in organization.invites" :key="index">
            {{ invite }}
            <button class="text-red-500" @click.prevent="removeInviteCode(invite)">×</button
            >{{ index != organization.invites.length - 1 ? ', ' : '' }}
          </span>
        </p>
      </section>
        ...
        <table class="w-full">
          ...
          >
            <tr>
              ...
              <th scope="col" class="px-3 py-2">Add permission</th>
            ...
            >
              ...
              <td class="px-3 py-2">
                <div class="flex flex-wrap gap-2">
                  <RemovableChip
                    ...
                    :key="permission"
                    @click-action="removePermission(role.id, permission)"
                  >
                    {{ permission }}
                  </RemovableChip>
                </div>
              ...
              <td class="px-3 py-2">
                <button
                  ...
                  class="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600 dark:hover:bg-red-700 transition"
                >
                  Delete
                </button>
              </td>
      ...
      <section class="w-full bg-white dark:bg-slate-800 shadow rounded-lg px-5 py-4">
        ...
        <button
          ...
          class="mb-2 px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 dark:hover:bg-red-800 transition"
        >
          Delete organization
        </button>
      </section>
