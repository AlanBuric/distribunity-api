...
const SessionRouter = Router()
  ...
  .post(
    ...
    async (
      ...
      const {
        rows: [dbFullUser],
        rowCount,
      } = await database.query('SELECT * FROM "user" WHERE email = $1', [email]);
  ...
  .post(
    '/refresh',
    async (request: Request, response: Response<UserLoginResponse | string>): Promise<any> => {
      ...
      const userId = await getUserIdFromToken(refreshToken, TokenType.REFRESH);
      const { passwordHash, ...user } = await UserService.getUserById(userId);
      const { token: accessToken, expiration } = await generateJwtToken(userId, TokenType.ACCESS);
  ...
  .delete('/logout', async (request: Request, response: Response): Promise<any> => {
  ...
  .use(
    async (
      ...
      if (await isTokenOnDenylist(accessToken)) {
        return response.status(StatusCodes.UNAUTHORIZED).send('Access token expired');
      }

      const userId = await getUserIdFromToken(accessToken, TokenType.ACCESS);
