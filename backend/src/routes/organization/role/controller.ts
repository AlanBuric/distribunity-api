...
export async function POST(
  ...
  const { rowCount } = await getDatabase().query('SELECT 1 FROM role WHERE name = $1', [name]);

  if (rowCount) {
    return response.status(StatusCodes.BAD_REQUEST).send('Role with that name already exists');
  }
...
export async function DELETE(
  ...
  const { rowCount } = await getDatabase().query('DELETE FROM role WHERE role_id = $1', [roleId]);
