...
import {Created, Item} from "../../../types/database-types.js";
...
const itemValidatorChain = [
    ...
    body("iconURL")
        .default("")
        .isURL()
        .withMessage("Icon URL isn't a valid URL"),
    body("unitPrice")
        .default(0)
        .isFloat({min: 0})
...
const ItemRouter = Router({mergeParams: true})
    .patch("/:itemId",
        param("itemId")
            .isUUID()
            .withMessage("Item ID isn't a valid UUID"),
        itemValidatorChain,
        ...
        (request: Request, response: Response<ErrorResponse, AuthorizedLocals>): any => {
            ...
            if (!item) {
                return response.status(StatusCodes.NOT_FOUND).send({error: `Item with ID ${itemId} not found`});
            }
    ...
    .post("",
        ...
        (request: Request, response: Response<ErrorResponse | WithUUID, AuthorizedLocals>): any => {
            const {organizationId, inventoryId, ...item} = matchedData<{
                organizationId: UUID;
                inventoryId: UUID;
            } & Exclude<Item, keyof Created>>(request);
            const organization = OrganizationService.getOrganizationById(organizationId);

            if (Object.values(organization.inventories)
                .map(inventory => inventory.name)
                .includes(item.name)) {
                return response.status(StatusCodes.BAD_REQUEST).send({error: `Inventory with the name ${item.name} already exists`});
            }
