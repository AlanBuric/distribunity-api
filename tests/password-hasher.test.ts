...
describe('PasswordHasher class functions', () => {
    ...
    const format = /^\$argon2id\$v=\d+\$m=\d+,t=\d+,p=\d+\$[A-Za-z0-9+/]+(?:=*)?\$[A-Za-z0-9+/]+(?:=*)?$/;
    ...
    it('should verify unique passwords even with different salts', async () => {
        ...
        const isValid1 = await passwordHasher.hashPassword(password1).then(hash => passwordHasher.verifyPassword(password1, hash));
        const isValid2 = await passwordHasher.hashPassword(password2).then(hash => passwordHasher.verifyPassword(password2, hash));
